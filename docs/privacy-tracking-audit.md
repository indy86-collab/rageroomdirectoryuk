...
| AdSense Auto ads | Google; global script in `app/layout.tsx` (`ca-pub-9868896840591922`) | Advertising, cookies/storage and ad-personalisation via Google's certified Privacy & messaging CMP (IAB TCF). | Script removed in the 20 Aug consent change. | No | Restored. UK/EEA/Swiss ad consent is Google's CMP, not the site analytics banner. Analytics consent never enables or disables ads. |
...
4. Confirm no URL matches `googletagmanager.com`, `google-analytics.com`, `cloudflareinsights.com` or `/_vercel/insights` before analytics choice/rejection. AdSense (`googlesyndication.com` / `doubleclick.net`) may load independently because ad consent is Google's CMP.
