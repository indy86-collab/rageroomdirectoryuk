...
| AdSense in-article | Google; loaded only by `InArticleAd` on editorial `/guides/*` and `/blog/*` URLs (`ca-pub-9868896840591922`) | Advertising, cookies/storage and ad-personalisation via Google's certified Privacy & messaging CMP (IAB TCF). Overlay/vignette Auto ads are disabled in the tag. | Global Auto ads script in the root layout. | No | Manual in-article unit on long articles only. UK/EEA/Swiss ad consent is Google's CMP, not the site analytics banner. Analytics consent never enables or disables ads. |
...
| Corporate Event Builder state | RageRoom Directory; `lib/corporate-event-builder/storage.ts` | Browser workspace in `localStorage` (guest key before purchase, purchase-scoped after payment), including user-entered plan content. | After using the free builder. Payment unlocks the PDF pack, not the planner. | Feature-led / contract | Not controlled by Analytics. |
...
| GetYourGuide activity widget | GetYourGuide; `GetYourGuideWidget.tsx` | After an explicit press, loads `widget.getyourguide.com` so visitors can browse complementary tours. GetYourGuide may then receive request, device and network information and set its affiliate cookie. | Not present. | No; optional feature | Click-to-load placeholder, same pattern as Maps. Independent of Analytics consent. Script is not in the root layout. |
...
No application cookie writer existed in the audited source. No video embeds, social widgets, CAPTCHA, session replay, fingerprinting, Meta Pixel, affiliate redirect tracker or remarketing tag was found. The GetYourGuide activity widget is click-to-load only and is not requested until the visitor presses its load button. Providers or the deployment edge can still set cookies independently of repository code; production browser audits and provider dashboards remain necessary.
...
4. Confirm no URL matches `googletagmanager.com`, `google-analytics.com`, `cloudflareinsights.com` or `/_vercel/insights` before analytics choice/rejection. AdSense (`googlesyndication.com` / `doubleclick.net`) may load on editorial guide/blog pages independently because ad consent is Google's CMP; it must not load on listing, checkout or game routes.
...
7. Confirm the GetYourGuide widget script (`widget.getyourguide.com`) is absent until its explicit load button is pressed.
