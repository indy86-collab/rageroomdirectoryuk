...
| GetYourGuide affiliate links | GetYourGuide; `NearbyActivitiesAffiliate.tsx` | Outbound `getyourguide.com` links with `partner_id` so visitors can browse complementary tours. No widget script is loaded. Following a link may let GetYourGuide set its affiliate cookie. | Not present. | No; optional feature | Always-visible sponsored outbound links. Independent of Analytics consent. Script is not in the root layout. |
...
No application cookie writer existed in the audited source. No video embeds, social widgets, CAPTCHA, session replay, fingerprinting, Meta Pixel, affiliate redirect tracker or remarketing tag was found. GetYourGuide is linked as outbound sponsored URLs only; the widget script is not requested. Providers or the deployment edge can still set cookies independently of repository code; production browser audits and provider dashboards remain necessary.
...
7. Confirm the GetYourGuide widget script (`widget.getyourguide.com`) is absent, and that outbound GetYourGuide links include `partner_id`.
